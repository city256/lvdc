...
class Environment:
    ...
    def compute_reward(self, action):
        ...
        #
        # if self.grid < -5 and action >= 1:
        #     reward = abs(reward) * 0.5
        #     reward2 = abs(reward2) * 0.5
        #     print(self.current_step, reward, action)
        # elif self.load - self.pv < -5 and action <= -1:
        #     reward += reward
        #     reward2 += reward2
        #     print(reward)
        #
        ...
        #     reward2 += reward2
        #
        # if self.peak_time == 0 and (self.load - self.pv) * 0.5 <= action/4 <= (self.load - self.pv) and self.workday==1:
        #     reward *= 0.6
        #     reward2 *= 0.6
        #     print(f'경부하: r={reward}, act={action}, grid={self.grid}')
        #     if self.next_day_holiday == 1 and action >= 0:
        #         reward *= 0.5
        #         reward2 *= 0.5
        #         print(f'다음날 휴일: {reward}')
        #
        #     # print('off',action/4)
        # # 평일 최대부하에 방전시 방전량은 grid의 50% 이상 방전시 보상
        # elif self.peak_time == 2 and action < 0 and -self.grid < action/4 < -self.grid * 0.3 and self.workday==1:
        #     reward *= 0.3
        #     reward2 *= 0.3
        #     print(f'최대부하: r={reward}, act={action}, grid={self.grid}')
        #
        #
        # if  self.next_day_holiday == 1 and self.peak_time == 0 and action > 0 and self.workday==0:
        #     reward += abs(reward) * 1.4
        #     reward2 += abs(reward2) * 1.4
        # elif self.next_day_holiday == 1 and self.peak_time == 0 and action < 0:
        #     reward += abs(reward) * 0.2
        #     reward2 += abs(reward) * 0.2
        #     # print(reward, action)

        # peak_time에 따른 보상 구조 추가
        # if self.peak_time == 0:  # Off-peak 시간대
        #     if action > 0:  # 충전
        #         reward += abs(reward) * 1.0  # 충전 보상 증가
        #     elif action < 0:  # 방전
        #         reward -= abs(reward) * 0.5  # 방전에 대한 패널티
        # elif self.peak_time == 2:  # Peak 시간대
        #     if action < 0:  # 방전
        #         reward += abs(reward) * 1.5  # 방전에 대한 보상 증가
        #     elif action > 0:  # 충전
        #         reward -= abs(reward) * 0.8  # 충전에 대한 패널티
        ...
        if self.done:
            reward2 += -env.total_cost / 1000
            reward2 += -env.usage_sum / 1000

        self.previous_total_cost = current_total_cost
        normal_reward = reward1_normalizer_ma.normalize(reward)
        normal_reward2 = reward2_normalizer_ma.normalize(reward2)
        w = 0.2
        return reward2 / 100000
...
def train_ppo(env, agent, memory, episodes=episodes, save_interval=save_interval, max_timesteps=max_timesteps, batch_interval=batch_interval):
    ...
    episode_rewards = []
    episode_total_costs = []  # 에피소드별 total_cost 저장
    best_total_cost = float('inf')  # 최소 total_cost를 추적
    best_model_path = "best_custom.pth"  # 최적 모델 저장 경로
    ...
    for episode in range(episodes):
        ...
        for t in range(max_timesteps):
            # 에이전트가 환경으로부터 액션을 선택
            action = agent.select_action(state, memory, env, state_info)
            ...
            memory.append_is_terminal(done)
        ...
        episode_total_costs.append(env.total_cost[0])
        ...
        if (episode + 1) % save_interval == 0:
            torch.save(agent.policy.state_dict(), f"custom_{episode + 1}.pth")

    # 학습 보상 및 total_cost 시각화
    ...
    # 에피소드 보상 그래프
    plt.subplot(2, 1, 1)
    plt.plot(np.array(episode_rewards).flatten(), label='Episode Reward')
    plt.xlabel('Episode')
    plt.ylabel('Total Reward')
    plt.title('Training Reward over Episodes(Custom)')
    plt.legend()
    ...
    plt.title('Training Total Cost over Episodes(Custom)')
    plt.legend()
    ...
    plot_results(
        ...
        f'Best Custom {best_episode_num}'
    )
...
d0907_csv = pd.read_csv('0907_0911.csv')
workday_csv = pd.read_csv('workday.csv')
holiday_csv = pd.read_csv('holiday.csv')
all_csv = pd.read_csv('ess_data.csv')
...
all_env = Environment(all_csv)
env = test_env

# 메모리 및 에이전트 초기화
memory = Memory()
ppo_agent = PPO(state_dim, action_dim, hidden_dim)

ppo_agent.policy.to(device)  # 모델을 GPU로 이동

# 학습 및 추론
train_ppo(env, ppo_agent, memory, max_timesteps=env.total_steps)
run_test(env, f"custom_{episodes}.pth", ppo_agent, 1, 0)
