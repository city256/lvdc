...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=256, max_action=250):
        ...
        # Actor network with additional layers and larger hidden dimensions
        ...
        # Critic network with a similar deeper structure
    ...
    def forward(self, state):
        ...
        if torch.isnan(mean).any():
            print("NaN detected in mean after actor network")
            print(f"Mean: {mean}")
            print(f"State: {state}")
...
def train_ppo(env, agent, memory, episodes=episodes, save_interval=save_interval, max_timesteps=max_timesteps, batch_interval = batch_interval):
    ...
    best_model_path = "best_po2.pth"  # 최적 모델 저장 경로
    ...
    for episode in range(episodes):
        ...
        if env.total_cost < best_total_cost:
            ...
            best_episode_data["total_cost"] = env.total_cost
            best_episode_data["usage_sum"] = env.usage_sum
            best_episode_data["demand"] = env.demand_sum
            best_episode_data["switch_sum"] = env.switch_sum
            best_episode_data["final_reward"] = env.episode_reward
        ...
        if (episode + 1) % save_interval == 0:
            torch.save(agent.policy.state_dict(), f"PO2_{episode + 1}.pth")
    ...
    plt.title('Training Reward over Episodes(PO2)')
    plt.legend()
    ...
    plt.title('Training Total Cost over Episodes(PO2)')
    plt.legend()

    plt.tight_layout()
    plt.show()

    print(f"\nBest episode cost information {best_episode_num}:")
    print(f"Total cost: {best_episode_data['total_cost']}")
    print(f"Usage sum: {best_episode_data['usage_sum']}")
    print(f"Demand cost: {best_episode_data['demand']}")
    print(f"Switch count: {best_episode_data['switch_sum']}")
    print(f"Final reward: {best_episode_data['final_reward']}")

    # 최적 모델의 결과 시각화
    print(f"Best model's total cost: {np.round(best_total_cost, -2)}")
    plot_results(
...
def run_test(env, model_path, agent, test=0, reference_value=0, scale = 1):
    ...
    for t in range(env.total_steps):
        ...
        adjusted_action = (action - reference_value) * scale
...
run_test(env, f"po2_{episodes}.pth", ppo_agent, 1, 0)
# run_test(env, f"po2_2400.pth", ppo_agent, 1, 0, 1)
