...
def train_ppo(env, agent, memory, episodes=episodes, save_interval=save_interval, max_timesteps=max_timesteps, batch_interval = batch_interval):
    ...
    best_model_path = "best_po.pth"  # 최적 모델 저장 경로
    ...
    for episode in range(episodes):
        ...
        if (episode + 1) % save_interval == 0:
            torch.save(agent.policy.state_dict(), f"PO_{episode + 1}.pth")
    ...
    plt.title('Training Reward over Episodes(PO)')
    plt.legend()
    ...
    plt.title('Training Total Cost over Episodes(PO)')
    plt.legend()
...
run_test(env, f"po_{episodes}.pth", ppo_agent, 1, 0)
# run_test(env, f"po_700.pth", ppo_agent, 1, 0, 1)
